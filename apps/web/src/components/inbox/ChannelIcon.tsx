import { type ChannelType } from '@/mock/inbox'
...
function InstagramSvg({ size }: { size: number }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} fill="#E84393">
...
function FacebookSvg({ size }: { size: number }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} fill="#4A7AFF">
...
export function ChannelIcon({ channel, size = 16, className }: ChannelIconProps) {
  return (
    <span className={cn('inline-flex items-center justify-center flex-shrink-0', className)}>
      {channel === 'whatsapp' && <WhatsAppSvg size={size} />}
      {channel === 'instagram' && <InstagramSvg size={size} />}
      {channel === 'facebook' && <FacebookSvg size={size} />}
    </span>
  )
}
